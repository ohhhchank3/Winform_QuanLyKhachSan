...
{
    public partial class ThongKeThuePhong : UserControl
    {
        ...
        {
            sql = "select count(idphong) as Tong ,day(ngaycheckin) as Thang from HoaDonPhong where month(ngaycheckin) = "+int.Parse(cmbthang.SelectedItem.ToString())+" group by day(Ngaycheckin)";
        }
        ...
        private void btnxemrieng_Click(object sender, EventArgs e)
        {
            ...
            if (btnxemtheothang.Checked == true)
            {
                ...
                string query = "select count(idphong) from hoadonphong where month(ngaycheckin) = "+int.Parse(lbthang.Text)+"";
                ...
                string query1 = "select count(idphong) from hoadonphong where month(ngaycheckin) = " +a112 + "";
            ...
            if (btnxemtheonam.Checked == true)
            {
               conn.Open();
                ...
                chart1.Series["Series1"].XValueMember = "Thang1";
              ...
              btnxemtheonam.Checked = false;
                // xử lý 
                lbthang.Text = cmbnam.SelectedItem.ToString();
                string query = "select count(idphong) from hoadonphong where year(ngaycheckin) = " + int.Parse(lbthang.Text) + "";
                SqlDataAdapter ad1 = new SqlDataAdapter(query, conn);
                DataTable dt1 = new DataTable();
                ad1.Fill(dt1);
                txtTongSoLuong.Text = Convert.ToString(dt1.Rows[0].ItemArray[0]);
                ...
                txthieusuat.Text = Convert.ToString(z) + "%";
                int a112 = int.Parse(lbthang.Text) - 1;
                string query1 = "select count(idphong) from hoadonphong where year(ngaycheckin) = " + a112 + "";
                SqlDataAdapter ad2 = new SqlDataAdapter(query1, conn);
                DataTable dt2 = new DataTable();
                ad2.Fill(dt2);
                string z1 = Convert.ToString(dt2.Rows[0].ItemArray[0]);
        ...
        private void btnlen_Click(object sender, EventArgs e)
        {
            if(lbthang.Text != "")
            {
                ...
                string sql22 = "select count(idphong) as Tong ,day(ngaycheckin) as Thang from HoaDonPhong where month(ngaycheckin) = " + tinhtoan + " group by day(Ngaycheckin)";
        ...
        private void ThongKeThuePhong_Load(object sender, EventArgs e)
        {
        ...
        private void btnxemtheonam_CheckedChanged(object sender, EventArgs e)
        {
            ...
            sql1 = "select count(idphong) as Tong1 ,Month(ngaycheckin) as Thang1 from hoadonphong where Year(ngaycheckin) = " + int.Parse(cmbnam.SelectedItem.ToString()) + " group by Month(Ngaycheckin) ";
        }
    }
}
